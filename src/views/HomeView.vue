      ...
      <SdCard bordered shadow="sm" size="lg">
        ...
        <div class="space-y-3">
          <SdText as="p" size="sm" color="muted">
            Účet je připojený k backendu. Mapu dlaždic otevřeš tlačítkem níže (vyžaduje lokální data
            <code class="text-xs">svarog-engine/outputs</code> při
            <code class="text-xs">npm run dev</code>).
          </SdText>
          <SdButton variant="primary" size="md" @click="router.push({ name: 'map' })">
            Otevřít mapu (fullscreen)
          </SdButton>
